import XCTest
@testable import Sneakerbox

@MainActor
final class SneakerboxTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(PairEntry(title: "Test", detail1: "A", detail2: "B", note: "", date: Date()))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(PairEntry(title: "ToDelete", detail1: "A", detail2: "B", note: "", date: Date()))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        while store.entries.count < Store.freeLimit {
            store.add(PairEntry(title: "Filler", detail1: "A", detail2: "B", note: "", date: Date()))
        }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(PairEntry(title: "Original", detail1: "A", detail2: "B", note: "", date: Date()))
