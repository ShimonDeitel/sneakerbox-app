...
struct ContentView: View {
    ...
    @State private var editingEntry: PairEntry?
            ...
            ZStack {
                ...
                if store.entries.isEmpty {
                    VStack(spacing: 12) {
                        ...
                        Text("Tap + to log your first pair.")
                            .font(Theme.font(15))
...
struct EntryFormView: View {
    ...
    let existingID: UUID?
    let onSave: (PairEntry) -> Void
    @FocusState private var focusedField: Field?

    enum Field { case title, detail1, detail2, note }

    init(entry: PairEntry?, onSave: @escaping (PairEntry) -> Void) {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(existingID == nil ? "Add Pair" : "Edit Pair")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = PairEntry(title: title, detail1: detail1, detail2: detail2, note: note, date: date)
